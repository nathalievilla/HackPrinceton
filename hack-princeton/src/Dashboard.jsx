              ...
              {loading && (
                <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
                  <div style={{ width: '100%', height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.08)', overflow: 'hidden' }}>
                    <div style={{ height: '100%', borderRadius: 2, background: '#185FA5', width: `${Math.max(currentJob?.progress ?? 0, 5)}%`, transition: 'width 0.5s ease' }} />
                  </div>
                  <p style={{ fontSize: 12, opacity: 0.5, textAlign: 'center' }}>{STAGE_LABELS[currentJob?.stage] || 'Starting analysis...'}</p>
                </div>
              )}
