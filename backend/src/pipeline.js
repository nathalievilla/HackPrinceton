...
async function runPipeline({ job_id, csvPath, jobDir, resultsPath, reportPath }) {
  ...
  // Save CSV upload info to Supabase
  ...
  // Save CSV upload summary now that we have the AI summary
  await db.updateCsvUploadSummary(job_id, summary.summary);
